...
class YakCenter: NSObject {
    ...
    */
    ...
    func voteOnYak(yak: Yak, isUpvote: Bool){
        if (voteDictionary[yak.snapshot!.key] == nil){ //check to see if vote is eligible
        ...
        let voteRef = yakRef.childByAppendingPath(yak.snapshot!.key + "/votes") //whtas up with this
        voteRef.runTransactionBlock({
            (currentData: FMutableData!) in
            let currentVotes = currentData.value as! Int
            
            currentData.value = isUpvote ? currentVotes + 1 : currentVotes - 1
